...
function payload(input: ProductInput) {
  return {
    slug:input.slug,sku:input.sku,name:input.name,short_description:input.shortDescription,
    description:input.description,image_url:input.imageUrl,price_cents:input.priceCents,
    compare_at_price_cents:input.compareAtPriceCents,stock_quantity:input.stockQuantity,
    status:input.status,featured:input.featured,
    price:(input.priceCents??0)/100,old_price:input.compareAtPriceCents===null?null:input.compareAtPriceCents/100,
    active:input.status==="ACTIVE",availability:input.status==="ACTIVE"&&input.stockQuantity>0?"available":"unavailable",
  };
}
...
export class SupabaseProductRepository implements ProductRepository {
  ...
  async create(input:ProductInput) { const {data,error}=await this.client.from("products").insert({id:input.slug,becoins:0,...payload(input)}).select(fields).single(); if(error) throw error; return map(data as ProductRow); }
