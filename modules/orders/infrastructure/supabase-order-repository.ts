...
const orderFields = "id, order_number, status, payment_method, installments, subtotal_cents, discount_cents, shipping_cents, total_cents, recipient_name, recipient_phone, postal_code, street, address_number, address_complement, neighborhood, city, state, paid_at, created_at";
...
const mapOrder = (row: OrderRow, items: readonly ItemRow[], history: readonly HistoryRow[]): Order => ({
  ...
  city:row.city, state:row.state, paidAt:row.paid_at, createdAt:row.created_at,
