...
export function CheckoutForm({ cart, customer, addresses }: Readonly<{ cart: Cart; customer: CheckoutCustomer | null; addresses: readonly CustomerAddress[] }>) {
  ...
  const shippingCents = cart.summary.qualifiesForFreeShipping ? 0 : 990;
  const estimatedTotal = cart.summary.subtotalCents - (paymentMethod === "PIX" ? pixDiscount : 0) + shippingCents;
  ...
  return <main className="checkout-page">
    ...
    <div className="checkout-shell">
      <form action={submit} className="checkout-content">
        ...
        <section className="checkout-card"><div className="section-title"><span>3</span><div><h2>Entrega</h2><p>Condição calculada pelo valor do carrinho</p></div></div><div className="delivery-box"><span aria-hidden="true">🚚</span><div><strong>{cart.summary.qualifiesForFreeShipping ? "Frete grátis conquistado" : "Frete fixo de R$ 9,90"}</strong><p>{cart.summary.qualifiesForFreeShipping ? "Você alcançou a meta de R$ 600,00." : `Faltam ${money.format(cart.summary.freeShippingRemainingCents / 100)} para ganhar frete grátis.`}</p></div></div></section>
      ...
      <aside className="order-summary"><div className="summary-heading"><div><small>RESUMO DO PEDIDO</small><h2>Sua rotina BEYOU</h2></div><Link href="/loja?cart=open">Editar</Link></div><div className="summary-items">{cart.items.map((item, index) => <article key={item.id}><div className={`summary-product product-${index % 3}`} aria-hidden="true"><small>BE</small><strong>{item.productName.replace(/^Be/i, "")}</strong></div><div className="summary-copy"><strong>{item.productName}</strong><span>1 unidade</span></div><b>{money.format(item.lineTotalCents / 100)}</b></article>)}</div><dl><div><dt>Subtotal</dt><dd>{money.format(cart.summary.subtotalCents / 100)}</dd></div>{paymentMethod === "PIX" && <div className="discount"><dt>Desconto Pix</dt><dd>- {money.format(pixDiscount / 100)}</dd></div>}<div><dt>Entrega</dt><dd>{cart.summary.qualifiesForFreeShipping ? "Grátis" : money.format(shippingCents / 100)}</dd></div><div className="total"><dt>Total</dt><dd>{money.format(estimatedTotal / 100)}</dd></div></dl><p><b aria-hidden="true">✦</b> Cuidar de você é a nossa fórmula.</p></aside>
    </div>
  </main>;
}
