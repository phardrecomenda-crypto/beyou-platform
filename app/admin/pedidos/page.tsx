...
type AdminOrderRow={id:string;order_number:number;user_id:string|null;status:string;payment_method:string|null;installments:number;total_cents:number;recipient_name:string;recipient_phone:string;city:string;state:string;tracking_code:string|null;shipping_carrier:string|null;created_at:string};
type AdminOrder=Omit<AdminOrderRow,"status"|"payment_method"|"user_id">&{user_id:string;status:OrderStatus;payment_method:"PIX"|"CREDIT_CARD"};
...
export default async function OrdersAdministrationPage(){
  ...
  const orders=((data??[]) as AdminOrderRow[]).filter((order)=>order.user_id).map((order)=>({
    ...order,
    user_id:order.user_id as string,
    status:order.status.toUpperCase() as OrderStatus,
    payment_method:(order.payment_method??"PIX").toUpperCase() as "PIX"|"CREDIT_CARD",
  }));
  ...
  const [itemsResult,profilesResult]=await Promise.all([
    ...
    userIds.length?admin.from("profiles").select("id, full_name").in("id",userIds):Promise.resolve({data:[] as ProfileRow[],error:null}),
  ]);
