...
function stepDate(order:Order,status:OrderStatus){if(status==="PAID")return order.paidAt;if(status==="SHIPPED")return order.shippedAt;if(status==="DELIVERED")return order.deliveredAt;return order.history.find((entry)=>entry.toStatus===status)?.createdAt??null}
...
export function OrderCard({ order }: Readonly<{order:Order}>) {
  return <article className="order-card"><div className="order-card-top"><div><small>PEDIDO</small><h2>#{order.orderNumber}</h2></div><StatusPill status={order.status} /></div><p>{dateTime.format(new Date(order.createdAt))} · {order.items.length} {order.items.length===1?"produto":"produtos"}</p><div className="order-card-items">{order.items.map((item)=><span key={item.id}>{item.quantity}× {item.productName}</span>)}</div>{order.trackingCode&&<p className="order-card-tracking">Rastreio disponível · {order.shippingCarrier??"Transportadora"}</p>}<div className="order-card-bottom"><strong>{money.format(order.totalCents/100)}</strong><Link href={`/pedidos/${order.id}`}>Acompanhar pedido <b>→</b></Link></div></article>;
}
...
export function OrderDetails({ order, confirmed=false }: Readonly<{order:Order;confirmed?:boolean}>) {
  ...
  return <main className="orders-page"><OrdersHeader /><div className="orders-container">
    ...
    {!(["CANCELLED","REFUNDED"] as OrderStatus[]).includes(order.status) && <ol className="order-timeline" aria-label="Andamento do pedido">{steps.map((step,index)=>{const timestamp=stepDate(order,step);return <li key={step} className={index<=currentIndex?"done":""}><span>{index<currentIndex?"✓":index+1}</span><div><b>{labels[step]}</b>{timestamp&&<small>{dateTime.format(new Date(timestamp))}</small>}</div></li>})}</ol>}
    <div className="order-detail-grid"><section className="order-panel"><h2>Produtos</h2>{order.items.map((item)=><article className="order-line" key={item.id}><div className="order-product-mark"><small>BE</small><b>YOU</b></div><div><strong>{item.productName}</strong><span>SKU {item.productSku} · {item.quantity} unidade</span></div><b>{money.format(item.lineTotalCents/100)}</b></article>)}<dl className="order-totals"><div><dt>Subtotal</dt><dd>{money.format(order.subtotalCents/100)}</dd></div>{order.discountCents>0&&<div className="discount"><dt>Desconto</dt><dd>- {money.format(order.discountCents/100)}</dd></div>}<div><dt>Entrega</dt><dd>{order.shippingCents===0?"Grátis":money.format(order.shippingCents/100)}</dd></div><div className="total"><dt>Total</dt><dd>{money.format(order.totalCents/100)}</dd></div></dl></section>
      ...
      {order.trackingCode&&<section className="order-panel order-tracking"><div><small>ACOMPANHE A ENTREGA</small><h2>Rastreamento disponível</h2><p><b>{order.shippingCarrier??"Transportadora responsável"}</b><br />Código: <strong>{order.trackingCode}</strong></p></div><Link href="/suporte">Preciso de ajuda</Link></section>}
    <div className="orders-actions"><Link href="/suporte">Falar com o suporte</Link><Link href="/pedidos">Todos os pedidos</Link><Link className="primary" href="/loja">Continuar comprando</Link></div>
  </div></main>;
}
